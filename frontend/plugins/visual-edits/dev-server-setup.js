...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all app.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*app\.sh$/)) {
        return true;
      }

      // Allow all appagent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*appagent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@app.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@app.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
